...
# them into the appropriate places in system memory. both setup.s and system
# has been loaded by the bootblock.
#
# This code asks the BIOS for memory/disk/other parameters, and puts them in a
# "safe" place: 0x90000-0x901ff, ie where the boot-block used to be. It is then
# up to the protected mode system to read them from there before the area is
...
_start:
	...
	int	$0x10
	mov	%dx, %ds:0	# save it in known place, console_init fetches
				# it from 0x90000.

	# Get memory size (get extended mem (outside 1MB), unit: kB),
	...
	int	$0x15		# starting at address 1MB
	...
	mov	$0x0000, %ax
	mov	%ax, %ds	    # ds = 0x0000
	lds	%ds:4 * 0x41, %si   # lds: Reads a pointer from memory, value
				    # stored in 4 * 0x41 is the address of hd0
				    # param table,
	mov	$INITSEG, %ax
	mov	%ax, %es	    # es = 0x9000
	mov	$0x0080, %di
	...
	je	has_disk1	# 02h: floppy with change-line 03h: hard disk

# if hd1(2nd disk) not exist, clear 16 byte from 0x9000:0090
no_disk1:
	mov	$INITSEG, %ax
	mov	%ax, %es	# es = 0x9000
	mov	$0x0090, %di
...
# each time move one segment size(64KB), totally move 8 segments
# 512KB (0x1000~0x8ffff)
do_move:
	mov	%ax, %es	# es = 0x0000 (destination segment)
	add	$0x1000, %ax
	cmp	$0x9000, %ax	# check (ax - 0x9000)
	jz	end_move	# jump if ax - 0x9000 = 0
	mov	%ax, %ds	# source segment
	sub	%di, %di
	sub	%si, %si
	mov 	$0x8000, %cx	# 0x8000 = 32K
	rep
		movsw
	jmp	do_move
...
end_move:
	...
	# -- new method -- (Fast Gate A20)
	inb     $0x92, %al	# read port 0x92 value and store to al
	testb   $02, %al	# if bit 1 already set, skip write
	jnz	a20_ok
	orb     $0b00000010, %al
	andb    $0b11111110, %al# bit 0 sometimes is write-only, and writing
			    	# one there causes a reset, so prevent it.
	outb    %al, $0x92	# write al value to port 0x92

# well, that went ok, I hope. Now we have to reprogram the interrupts :-(
# we put them right after the intel-reserved hardware interrupts, at
# int 0x20-0x2f. There they won't mess up anything. Sadly IBM really messed this
# up with the original PC, and they haven't been able to correct it. Thus the
# BIOS puts interrupts at 0x08-0x0f, which is used for the internal hardware
# interrupts as well. We just have to reprogram the 8259's, and it isn't fun.

# 8259A-1 port: 20h~21h, 8259A-2 port: a0h~a1h
a20_ok:
	# ICW1(set trigger type) - need write to 20h, a0h
	...
	# ICW2(set interrupt number) - need write to 21h, a1h
	...
	# ICW3(set relationship of 8259A-1 and 8259A-2) - need write to 21h, a1h
	...
	# ICW4
	mov	$0x01, %al		# 8086 mode, need send EOI to re-enable
	out	%al, $0x21		# interrupt
	.word	0x00eb, 0x00eb
...
# well, that certainly wasn't fun :-(. Hopefully it works, and we don't need
# BIOS anymore (except for the initial loading :-). The BIOS-routine wants lots
# of unnecessary data, and it's less "interesting" anyway. This is how REAL
# programmers do it.
#
# Well, now's the time to actually move into protected mode. To make things as
# simple as possible, we do no register set-up or anything, we let the
...
# address 0x00000, in 32-bit protected mode.
	...
	ljmp	$0x0008, $0	# select for 1st code segment(  001: 0:00) 
				# jmp offset 0 of 1st code segment in gdt
